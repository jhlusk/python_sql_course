...
DATABASE_PROMPT = "Enter the DATABASE_URL value or leave it empty to load from .env file: "
...
def prompt_vote_poll():
    poll_id = int(input("Enter the poll you would like to vote on: "))
    poll = Poll.get(poll_id)

    _print_poll_options(poll.options)

    option_id = int(input("Enter option you'd like to vote for: "))
    username = input("Enter the username you'd like to vote for: ")
...
def randomize_poll_winner():
    ...
    option_id = int(input("Enter which is the winning option, we'll pick a random winner from the voters: "))
